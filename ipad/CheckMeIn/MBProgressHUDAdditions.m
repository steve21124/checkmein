...
+ (UIView*)activityIndicatorView {
    ...
    activityIndicator.frame = CGRectMake(0.5 * (customView.frame.size.width - activityIndicator.size.width),
                                         0.5 * (customView.frame.size.height - activityIndicator.size.height),
                                         activityIndicator.size.width, activityIndicator.size.height);
...
+ (MBProgressHUD *)showHUDAddedTo:(UIView *)view withImage:(UIImage*)image text:(NSString*)text subText:(NSString*)subText minShowTime:(NSTimeInterval)minShowTime {
    MBProgressHUD* HUD = [[MBProgressHUD alloc] initWithFrame:TTScreenBounds()];
